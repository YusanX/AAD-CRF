...
from aadcrf.training.train_avgc_icassp2023_post import (
    ExperimentConfig,
    load_config,
    run_experiment,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train ICASSP2023 model (CE only) + HMM post-processing on AVGC."
        )
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(
            PROJECT_ROOT / "configs" / "avgc_icassp2023_post.yaml"
        ),
