...
from aadcrf.training.train_avgc_lstm_post import load_config, run_experiment


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train LSTMTracker with CE only, then apply fixed HMM post-processing "
            "on AVGC AAD data."
        )
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "configs" / "avgc_lstm_post.yaml"),
