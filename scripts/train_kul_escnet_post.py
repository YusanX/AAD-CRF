...
from aadcrf.training.train_kul_escnet_post import load_kul_config, run_kul_experiment


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train ESCNet with CE only, then apply fixed HMM "
            "post-processing on KUL AAD data."
        )
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(PROJECT_ROOT / "configs" / "kul_escnet_post.yaml"),
